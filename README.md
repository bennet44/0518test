...
- **價格、技術指標與基本面**（單一股票）：K 線圖、SMA20/50、布林通道、RSI、MACD、成交量，與該股票的基本面財務（P/E、P/B、EPS、營收/盈餘成長率、ROE、淨利率、股息率），分頁內輸入股票代號（預設 AAPL）與時間範圍即可連動更新，頁面最下方附上該股票今日／昨日的中文新聞標題（透過 Google News RSS 抓取）
...
- **買賣建議**：分頁內可設定時間範圍、無風險利率與建議買賣標的數量 (Top N，可選 1/5/10/15，預設 5)。篩選範圍為「美股交易量前 30 大（觀察名單）」與「S&P 500 成分股」的聯集，以基金經理人角度綜合期間報酬率、Sharpe Ratio、價格趨勢、估值水準、近兩日中文新聞情緒（標題關鍵字判斷）計算評分，列出建議買入／賣出的前 N 項，並估算買入價／賣出價（假設 3~5% 價格波動）
