...
以資料分析師視角打造的美股互動看板，使用 [Streamlit](https://streamlit.io/) + [yfinance](https://github.com/ranaroussi/yfinance) 免費資料。

## 功能

- **價格、技術指標與基本面**（單一股票）：K 線圖、SMA20/50、布林通道、RSI、MACD、成交量，與該股票的基本面財務（P/E、P/B、EPS、營收/盈餘成長率、ROE、淨利率、股息率），分頁內輸入股票代號即可連動更新（預設 AAPL），頁面最下方附上該股票今日／昨日的中文新聞標題（透過 Google News RSS 抓取）
- **多股比較、相關性與風險統計**：輸入逗號分隔的股票代號（預設 `AAPL, OKLO`，留空代表全部 S&P 500 成分股），同時查看累積報酬比較、日報酬相關係數矩陣（熱力圖）、年化報酬率／年化波動率／Sharpe Ratio／最大回撤／VaR 與日報酬分布
- **買賣建議**：篩選範圍為「美股交易量前 30 大（觀察名單）」與「S&P 500 成分股」的聯集，以基金經理人角度綜合期間報酬率、Sharpe Ratio、價格趨勢、估值水準、近兩日中文新聞情緒（標題關鍵字判斷）計算評分，列出建議買入／賣出的前 N 項（N 可選 1/5/10/15，預設 5），並估算買入價／賣出價（假設 3~5% 價格波動）
...
在側邊欄選擇時間範圍、無風險利率與建議買賣標的數量，再切換三個分頁查看分析結果。第二個分頁的比較代號欄位留空則代表「全部」，會自動帶入 S&P 500 全部成分股（清單即時從 Wikipedia 抓取，若無網路則退回一份內建的知名成分股清單）；標的數量較多時，比較圖表會自動裁切前 30 檔以維持可讀性與效能，風險統計表則仍處理全部標的。
